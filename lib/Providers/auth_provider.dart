import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:http/http.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:tailor_app/Modals/user_modal.dart';
import 'package:tailor_app/Reusable%20components/app_bar.dart';
import 'package:tailor_app/Urls/url_holder_loan.dart';

class AuthProvider with ChangeNotifier {
  String userAvailable = "";
  UserModal? userLogined;

  Future<bool> sendOtp(String phoneNos) async {
    var headers = {
      'Content-Type': 'application/json',
      'Accept': 'application/json'
    };
    var request =
        Request('POST', Uri.parse('${UrlHolder.baseUrl}${UrlHolder.login}'));
    request.body = json.encode({"mobile_number": phoneNos});
    request.headers.addAll(headers);

    StreamedResponse response = await request.send();

    if (response.statusCode == 200) {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];
      final responseDataUserAvail = json.decode(xyz)["user_available"];

      userAvailable = responseDataUserAvail.toString();

      message(responseData);
      return true;
    } else {
      final xyz = await response.stream.bytesToString();
      final responseData = json.decode(xyz)["msg"];
      message(responseData);
      return false;
    }
  }

  Future<bool> validateOtp(String phoneNos, String otp) async {
    var headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/json'
    };
    print(phoneNos);
    print(otp);
    var request = Request(
        'POST', Uri.parse('${UrlHolder.baseUrl}${UrlHolder.validateOtp}'));
    request.body = json.encode({"mobile_number": phoneNos, "otp": otp});
    request.headers.addAll(headers);

    StreamedResponse response = await request.send();

    if (response.statusCode == 200) {
      final prefs = await SharedPreferences.getInstance();
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];
      final authToken = json.decode(xyz)["data"]["access_token"];

      await prefs.setString(
        "userToken",
        authToken.toString(),
      );

      message(responseData);
      return true;
    } else {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];

      message(responseData);
      return false;
    }
  }

  Future<bool> resendOtp(String phoneNos) async {
    var request = Request('GET',
        Uri.parse('${UrlHolder.baseUrl}${UrlHolder.resendOtp}/$phoneNos'));

    StreamedResponse response = await request.send();

    if (response.statusCode == 200) {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];
      message(responseData);
      return true;
    } else {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];
      message(responseData);
      return false;
    }
  }

  Future updateProfile<bool>(String personName, String address) async {
    print(personName);
    print(address);
    final prefs = await SharedPreferences.getInstance();
    var _accessToken = await prefs.getString("userToken");
    var headers = {
      'Accept': 'application/json',
      'Authorization': 'Bearer $_accessToken',
      'Content-Type': 'application/json'
    };
    var request = Request(
        'POST', Uri.parse('${UrlHolder.baseUrl}${UrlHolder.updateUser}'));
    request.body = json
        .encode({"contact_person_name": "$personName", "address": "$address"});
    request.headers.addAll(headers);

    StreamedResponse response = await request.send();

    if (response.statusCode == 200) {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];
      await viewProfile();
      message(responseData);
      return true;
    } else {
      final xyz = await response.stream.bytesToString();
      final responseData = json.decode(xyz)["msg"];
      message(responseData);
      return false;
    }
  }

  Future viewProfile() async {
    final prefs = await SharedPreferences.getInstance();
    var _accessToken = await prefs.getString("userToken");

    var headers = {
      'Authorization': 'Bearer $_accessToken',
      'Cookie':
          'XSRF-TOKEN=%3D; laravel_session=%3D'
    };
    var request = Request(
        'GET', Uri.parse('${UrlHolder.baseUrl}${UrlHolder.viewProfile}'));

    request.headers.addAll(headers);

    StreamedResponse response = await request.send();

    UserModal? demoUserModal;

    if (response.statusCode == 200) {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["data"];

      demoUserModal = UserModal(
        address: responseData["address"] ?? "",
        contact_person_name: responseData["contact_person_name"] ?? "",
        current_wallet_points: responseData["current_wallet_points"] ?? "",
        id: responseData["id"].toString(),
        image: responseData["image"] ?? "",
        mobile_number: responseData["mobile_number"] ?? "",
      );

      userLogined = demoUserModal;
      notifyListeners();
    } else {
      final xyz = await response.stream.bytesToString();

      final responseData = json.decode(xyz)["msg"];
      message(responseData);
    }
  }
}
